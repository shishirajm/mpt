# CLAUDE.md — Mac Precitec India Private Limited website
...
- [x] ~~Open Graph share image~~ — `src/assets/brand/og-home.png` exists (placeholder). **Temporary:** every page's `og:image` currently points at the CloudFront domain (`d3fflgswcmyi18.cloudfront.net`) instead of `www.macprecitecindia.com`, so link previews (WhatsApp etc.) work during pre-launch testing while DNS/cert are still pending. Marked `<!-- TEMP -->` inline on every page — **revert to `www.macprecitecindia.com` once the domain cutover is live**, or previews will silently break again in the other direction.
